...
var checkAddItem = function(handler, data, nextItem){
  ...
  /*
  handler.metrics.forEach(function(metric){
    if((!metric.disabled) && (sift(metric.filter, aData).length>0)){
      matches.push(metric);
    }
  });
  */
...
//TODO: Need to actually handle pushing the match to Bosun here
    var metric = {
      name: match.name,
...
Handler.prototype.sendToBosun = function(pkt){
  ...
  console.log(util.inspect(pkt, {depth: null, colors: true}));
  Wreck.post(
    this.putUrl,
    {
